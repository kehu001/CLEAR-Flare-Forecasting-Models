...
class New_SampleConstruction:
    ...
    def samples_from_harp(self, flare_list, sharps_all, sharp_params=params, input_window=24, lead_window=0, forecasting_window=24):
        ...
        #harpnums_unique = sharps_all['HARPNUM'].unique().copy()
        for harpnum, oneharp in sharps_all.groupby(
            'HARPNUM',
            sort=False,
            dropna=False
        ):
            print('Processing HARPNUM:', repr(harpnum))

            if pd.isna(harpnum):
                print("Skipping missing HARPNUM")
                continue

            oneharp = (
                oneharp.copy()
                .sort_values(by='T_REC')
                .drop_duplicates(subset='T_REC', keep='first')
            )

            #print(oneharp.shape[0], 'rows of data for HARPNUM', harpnum)

            if oneharp.empty:
                raise RuntimeError(
                    f"Unexpected empty DataFrame for HARPNUM {harpnum}"
                )

            ar_values = oneharp['NOAA_AR'].dropna()

            if ar_values.empty:
                print(f"Skipping HARPNUM {harpnum}: NOAA_AR is entirely missing")
                continue
            ...
            if ar == 0: # non valid AR number
                ...
                if flares_sub.shape[0] == 0: # no flares in the AR
                    ...
                    flares_sub = flares_sub[(flares_sub['peak_time'] >= time1) & (flares_sub['peak_time'] <= time2 
                                                                                  + pd.Timedelta(str(lead_window) + ' hours')
                                                                                  + pd.Timedelta(str(forecasting_window) + ' hours'))]
...
# 2026/7 update: allow two tasks: Task1: M/X vs A/B; Task2: M/X vs A/B/C
def get_samples(inputs_profiles, labels, purpose, time1, time2):
    ...
    mag = np.array([label[0] for label in labels], dtype=str)
    ...
    if purpose == "Task1":
        ...
        P_inputs = np.array([inputs_profiles[i]['vobj'] for i in range(len(inputs_profiles)) if mag[i] in ['M', 'X']])
        N_inputs = np.array([inputs_profiles[i]['vobj'] for i in range(len(inputs_profiles)) if mag[i] in ['A', 'B', 'C']])
        return P_inputs, N_inputs
